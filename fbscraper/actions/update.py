...
from config import DEFAULT_IS_LOGINED, DEFAULT_MAX_TRY_TIMES, \
    ...
    DEFAULT_NEXT_SNAPSHOT_AT_INTERVAL, STATUS_SUCCESS


class UpdateCrawler:
    def __init__(self, article_url, article_id, db, browser, timeout, max_try_times=DEFAULT_MAX_TRY_TIMES,
    ...
    def crawl_and_save(self):
        ...
        if is_located:
            try:
                ...
                logger.error('[post_crawler] failed to expand comment for unknown reason')
                should_relocate_for_loaded_comment = False
                pass
        else:
            should_relocate_for_loaded_comment = False
        ...
        raw_html = helper.get_html(self.post_node) if self.post_node else self.get_post_raw_html(self.browser.page_source)
        self.save(raw_html)
    ...
    def enter_site(self):
        ...
        success_status = f'crawler: successful to enter site with url "{self.article_url}"'
    ...
    def turn_off_comment_filter(self):
        ...
        logger.debug(f'crawler: clicked comment filter button with selector="{filter_menu_link_selector}" \n')
        helper.move_to_element_by_selector(filter_menu_selector, self.browser)
        logger.debug(f'crawler: comment filter menu is shown with selector="{filter_menu_selector}" \n')
        helper.click_with_move(unfiltered_option_selector, self.browser)
        logger.debug(f'crawler: clicked comment filter "RANKED_UNFILTERED" with selector="{unfiltered_option_selector}" \n')
